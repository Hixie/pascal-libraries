...
uses
   hashtable, genericutils, sysutils;
...
type
   ...
   TFixupHashTable = specialize THashTable<PtrUInt, Pointer, PtrUIntUtils>;
   ...
   TReadStream = class
    protected
     ...
     var
      ...
      function GetFilename(): RawByteString;
    ...
    public
      ...
      property Filename: RawByteString read GetFilename;
   end;
   ...
   TWriteStream = class
    ...
    public
      ...
      procedure WriteString(Value: RawByteString);
...
procedure RegisterStorableClassSynonym(AClassName: RawByteString; RestoreClass: StorableClass);
function GetRegisteredClass(AClassName: RawByteString): StorableClass;
function GetRegisteredClasses(Superclass: TClass): TRawByteStringArray;

procedure StoreObjectToFile(FileName: RawByteString; Value: TStorable; Version: Cardinal);
...
type
   TStorableClassesHashTable = specialize THashTable<RawByteString, StorableClass, RawByteStringUtils>;
...
{$IFOPT C+}
function HasStorableClassName(AClass: StorableClass): Boolean;
begin
   Result := Pos('$', AClass.ClassName) = 0;
end;
{$ENDIF}
...
begin
   ...
   {$IFOPT C+} Assert(HasStorableClassName(AClass), AClass.ClassName + ' has a mangled name; storing it would not be portable across compiler versions'); {$ENDIF}
...
var
   ...
   AClass: TClass;
begin
   SetLength(Result, RegisteredClasses.Count); {BOGUS Warning: Function result variable of a managed type does not seem to be initialized}
   Index := 0;
   for Name in RegisteredClasses do
   begin
      AClass := RegisteredClasses[Name];
      if ((AClass = Superclass) or (AClass.InheritsFrom(Superclass))) then
      begin  
         Result[Index] := Name;
         Inc(Index);
      end;
...
function TReadStream.ReadString: RawByteString;
var
   Length: Cardinal;
begin
   VerifyFieldType(btString);
   Result := '';
   Length := ReadCardinal();
...
var
   RestoreClassName: RawByteString;
begin
   VerifyFieldType(btClass);
   RestoreClassName := ReadString();
...
begin
   ...
   MethodData^.MethodName := ReadString();
...
function TReadStream.GetFilename(): RawByteString;
begin
...
procedure TWriteStream.WriteString(Value: RawByteString);
var
   Result: Cardinal;
begin
   WriteFieldType(btString);
   WriteCardinal(Length(Value));
   // XXX should also write dynamic code page
   if (Length(Value) > 0) then
   begin
      Result := 0;
      BlockWrite(FOutput, Value[1], Length(Value), Result); // $R-
...
begin
   ...
   Assert(Length <= High(LongInt)); // BlockWrite(3 args) last arg is LongInt, not Cardinal
   BlockWrite(FOutput, Buffer, Length); // $R-
end;
...
var
   ResolvedMethodName: RawByteString;
begin
   ...
   begin
      ...
      WriteString(ResolvedMethodName);
   end
   else
   begin
      WriteString('');
...
initialization
   RegisteredClasses := TStorableClassesHashTable.Create(@RawByteStringHash32);
finalization
